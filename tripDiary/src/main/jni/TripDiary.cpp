...
static double distFrom(double lat1, double lng1, double lat2, double lng2) {
    ...
    dLat = (lat2 - lat1) * M_PI / 360;
    dLng = (lng2 - lng1) * M_PI / 360;
    dist = sin(dLat) * sin(dLat) +
           cos(M_PI * lat1 / 180) * cos(M_PI * lat2 / 180) * sin(dLng) * sin(dLng);
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    rTotalTime = env->GetFieldID(recordClass, "totalTime", "J");
    ...
    allRecordActivity_onTrackFinished = env->GetMethodID(allRecordActivityClass, "onTrackFinished",
                                                         "([D[D)V");

    return JNI_VERSION_1_6;
}
JNIEXPORT jboolean Java_com_yupog2003_tripdiary_data_GpxAnalyzer2_parse(JNIEnv *env, jobject thiz,
                                                                        jstring gpxPath,
                                                                        jobject cache,
                                                                        jint timezoneO) {
    stop = false;
    ...
    while (getline(fis, s)) {
        ...
        if (contains(s, "<trkpt")) {
            ...
            float altitude = myatof(
                    s.substr(s.find(">") + 1, s.rfind("<") - s.find(">") - 1).c_str());
    ...
    fis.close();
    int trackSize = (int) track.size();
    int timesSize = (int) times.size();
...
JNIEXPORT bool Java_com_yupog2003_tripdiary_data_GpxAnalyzer2_getCache(JNIEnv *env, jobject thiz,
                                                                       jstring cachePath,
                                                                       jobject cache) {
    ...
    while (getline(fis, s)) {
        ...
        if (blockIndex == blockSize - 1 || index == size - 1) {
            env->SetDoubleArrayRegion(latitudes, index - blockIndex, blockIndex + 1,
                                      latitudesArray);
            env->SetDoubleArrayRegion(longitudes, index - blockIndex, blockIndex + 1,
                                      longitudesArray);
...
JNIEXPORT bool Java_com_yupog2003_tripdiary_AllRecordActivity_parse(JNIEnv *env, jobject thiz,
                                                                    jstring gpxPath,
                                                                    jobject record) {
    stop = false;
    ...
    int trackSize = (int) track.size();
    if (timeStrs.size() > 0)
        totalSeconds = getSecondsFromString(timeStrs[timeStrs.size() - 1]) -
                       getSecondsFromString(timeStrs[0]);
    jdoubleArray lats = env->NewDoubleArray(trackSize);
    jdoubleArray lngs = env->NewDoubleArray(trackSize);
    int blockSize = trackSize / 100;
    if (blockSize == 0)
        blockSize = trackSize;
    int blockIndex;
    ...
    for (int i = 0; i < trackSize; i++) {
        ...
        longitudes[blockIndex] = (jdouble) track[i].lng;
        if (blockIndex == blockSize - 1 || i == trackSize - 1) {
            env->SetDoubleArrayRegion(lats, i - blockIndex, blockIndex + 1, latitudes);
            env->SetDoubleArrayRegion(lngs, i - blockIndex, blockIndex + 1, longitudes);
        }
    }
    env->CallVoidMethod(thiz, allRecordActivity_onTrackFinished, lats, lngs);
    env->DeleteLocalRef(lats);
    env->DeleteLocalRef(lngs);
    env->SetLongField(record, rTotalTime, (jlong) (static_cast<long int>(totalSeconds)));
