...
public class Trip implements Comparable<Trip> {
    ...
    public DocumentFile generateGPX(DocumentFile gpxFile, GenerateKMxListener listener) {
        if (pois == null || cache == null || gpxFile == null || note == null)
            return null;
        try {
            ...
            bw.write(" <trk>\n");
            bw.write("  <trkseg>\n");
            BufferedReader br = new BufferedReader(new InputStreamReader(this.gpxFile.getInputStream()));
            String s;
            int totalLength = (int) this.gpxFile.length();
            int count = 0;
            while ((s = br.readLine()) != null) {
                ...
                if (listener != null) {
                    listener.onProgressChanged(count, totalLength);
                }
            }
            br.close();
            bw.write("  </trkseg>\n");
    ...
    public DocumentFile generateKML(DocumentFile kmlFile, GenerateKMxListener listener) {
        ...
        try {
            ...
            bw.write("<Document>\n");
            bw.write("<name><![CDATA[" + tripName + "]]></name>\n");
            bw.write("<description><![CDATA[" + note + "]]></description>\n");
            ...
            int length = cache.getTrackLength();
    ...
    public DocumentFile generateKMZ(DocumentFile kmzFile, GenerateKMxListener listener) {
        try {
            ...
            for (int i = 0; i < poiLength; i++) {
                ...
                bw.write("<name><![CDATA[" + poi.title + "]]></name>\n");
                bw.write("<styleUrl>#iconColor</styleUrl>\n");
                bw.write("<description><![CDATA[" + descriptionBuilder.toString() + "]]></description>\n");
                bw.write("<Point>\n");
                bw.write("<coordinates>" + String.valueOf(poi.longitude) + "," + String.valueOf(poi.latitude) + "," + String.valueOf(poi.altitude) + "</coordinates>\n");
                bw.write("</Point>\n");
                bw.write("</Placemark>\n");
